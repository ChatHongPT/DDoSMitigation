...
|속성명|속성|
|:----:|:----:|
&redef 전역 상수를 다시 정의하거나 유형을 확장합니다.
&priority 이벤트 핸들러 또는 후크에 대한 우선 순위를 지정합니다.
&log 로그에 기록할 레코드 필드를 표시합니다.
&optional 레코드 필드 값이 누락되도록 합니다.
&default 기본값을 지정합니다.
&add_func 각각에 대해 호출할 함수를 지정합니다. redef +=.
&delete_func 과 같음 &add_func, 을 제외하고는 redef -=.
&expire_func 컨테이너 요소가 만료되면 호출할 함수를 지정합니다.
&read_expire 읽기 시간 초과 간격을 지정합니다.
&write_expire 쓰기 시간 초과 간격을 지정합니다.
&create_expire 생성 시간 초과 간격을 지정합니다.
&on_change 설정/테이블 변경을 호출할 함수 지정
&raw_output raw 모드에서 파일을 엽니다(chars.는 빠져 나오지 않습니다).
&error_handler 리포터 프레임워크 이벤트에 내부적으로 사용됩니다.
&type_column 유형에 대한 입력 프레임워크에서 사용됩니다.
&backend 테이블 지속성/동기화에 사용됩니다.
&broker_store 테이블 지속성/동기화에 사용됩니다.
&broker_allow_complex_type 테이블 지속성/동기화에 사용됩니다.
&ordered 테이블 및 집합의 예측 가능한 멤버 반복에 사용됩니다.
&deprecated 식별자를 사용하지 않음으로 표시합니다.
&is_assigned "정의되기 전에 사용" 경고 억제 zeek -u 분석.
&is_used 의 사용 부족 경고 억제 zeek -u 분석.
&group 이벤트 처리기 및 후크에 이벤트 그룹을 주석으로 달 수 있습니다.

타입 목록 및 사용법 : https://docs.zeek.org/en/v6.0.0/script-reference/types.html
연산자(프로그래밍 언어와 거의 동일함) : https://docs.zeek.org/en/v6.0.0/script-reference/operators.html
